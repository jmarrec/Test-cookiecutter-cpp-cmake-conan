...
```bash
docker build -f ./.devcontainer/Dockerfile --tag=test_cookiecutter_cpp_cmake_conan:latest .
docker run -it test_cookiecutter_cpp_cmake_conan:latest
```
...
```bash
docker build --tag=test_cookiecutter_cpp_cmake_conan:latest --build-arg GCC_VER=11 --build-arg LLVM_VER=15 .
```
...
```bash
docker build --tag=test_cookiecutter_cpp_cmake_conan:latest --build-arg USE_CLANG=1 .
```
...
docker run -it \
	...
	test_cookiecutter_cpp_cmake_conan:latest
```
